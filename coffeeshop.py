...
con = sqlite3.connect('orders5.db', check_same_thread=False)
cursor = con.cursor()
cursor.execute('''
...
@app.route('/')
def home():
    return render_template('coffeeindex.html', menu=menu)
...
def order():
    ...
    if coffee:
        ...
        return render_template('coffeeorder.html', coffee=coffee, quantity=quantity, total_price=total_price)
